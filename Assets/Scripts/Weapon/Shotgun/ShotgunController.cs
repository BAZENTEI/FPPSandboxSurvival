...
public class ShotgunController : GunControllerBase {
    ...
    protected override void PlayFireEffect() {
        GameObject temp = Instantiate<GameObject>(Effect, M_WeaponViewBase.M_MuzzlePos.position, Quaternion.identity);
    ...
    private IEnumerator CreateBullet() {
        for(int i = 0;i < pelletCapactiy; i++){
            ...
            GameObject tempBullet = Instantiate<GameObject>(m_ShotgunView.Prefab_Bullet, m_ShotgunView.M_MuzzlePos.position, Quaternion.identity);
            //
            tempBullet.GetComponent<ShotgunBullet>().Shoot(m_ShotgunView.M_MuzzlePos.forward + offset, 2000, Damage / pelletCapactiy, Hit);
